...
def run_backtest(df, risk_tolerance="Medium"):
    ...
    for i in range(1, len(df)):
        ...
        above_fvg = not pd.isna(df['FVG_Support'].iloc[i]) and close > float(df['FVG_Support'].iloc[i])
        bullish_struct = bool(hh.iloc[i] and hl.iloc[i])
        bearish_struct = (float(df['High'].iloc[i]) < float(df['High'].iloc[i-1])) and (float(df['Low'].iloc[i]) < float(df['Low'].iloc[i-1]))
        ...
        # Apply daily return only if we have previous price
        if i == 0:
            daily_ret = 1.0
        else:
            prev_close = float(df['Close'].iloc[i-1])
            daily_ret = close / prev_close if prev_close != 0 else 1.0
    ...
    df_bt['Buy_and_Hold'] = df['Close'].apply(lambda x: float(x) / first_close)
...
st.markdown("Institutional-grade analysis: **FVG, Liquidity, Market Structure, RSI** + **Backtesting**")
...
if not ticker:
    st.warning("Please enter a valid stock ticker (e.g., AAPL, NVDA, TSLA).")
    st.stop()

@st.cache_data(ttl=3600)
def load_data(ticker, period):
    try:
        data = yf.download(ticker, period=period, interval="1d", progress=False, timeout=10)
        return data
    except Exception:
        return pd.DataFrame()
...
if df is None or df.empty:
    st.error(f"❌ No data returned for **{ticker}** with period **{timeframe}**. Try a valid ticker like `AAPL` or `NVDA`.")
    st.stop()

expected_cols = ['Open', 'High', 'Low', 'Close', 'Volume']
missing_cols = [col for col in expected_cols if col not in df.columns]
if missing_cols:
    st.error(f"❌ Missing expected columns: {missing_cols}. Data may be corrupted.")
    st.stop()
...
# Backtesting
if enable_backtest and len(df) > 20:
    try:
        with st.spinner("Running backtest..."):
            df_bt, perf = run_backtest(df)
        
        st.subheader("📊 Backtest Results (Last 2 Years)")
        ...
        fig_bt.update_layout(title="Equity Curve: Strategy vs Buy & Hold", height=350, showlegend=True)
        st.plotly_chart(fig_bt, use_container_width=True)
    except Exception as e:
        st.warning(f"⚠️ Backtest skipped due to insufficient or unstable data.")
...
last_idx = -1
bullish = bool(hh.iloc[last_idx] and hl.iloc[last_idx])
rsi_val = float(df_live['RSI'].iloc[last_idx])
above_fvg = len(fvg_bull) > 0 and (latest_price > float(df_live['Low'].iloc[fvg_bull[-1] - 2]))
...
st.subheader(f"🧠 AI Recommendation: {ticker}")
st.markdown(f"### **Action: {decision}**")
st.markdown(f"- **RSI**: {rsi_val:.1f}")
st.markdown(f"- **Market Structure**: {'✅ Bullish (HH+HL)' if bullish else '⚠️ Not Bullish'}")
...
for i in fvg_bull[-3:]:
    if 2 <= i < len(df):
        fig.add_shape(
            type="rect", x0=df.index[i-2], x1=df.index[i],
...
for i in fvg_bear[-3:]:
    if 2 <= i < len(df):
        fig.add_shape(
            type="rect", x0=df.index[i-2], x1=df.index[i],
            y0=float(df['Low'].iloc[i]), y1=float(df['High'].iloc[i-2]),
...
fig.update_layout(
    height=650,
    title=f"{ticker} Price Action & RSI",
    xaxis_rangeslider_visible=False,
    showlegend=False
)
st.plotly_chart(fig, use_container_width=True)

st.caption("💡 Based on institutional concepts (FVG, liquidity, market structure). Not financial advice. For educational use only.")
